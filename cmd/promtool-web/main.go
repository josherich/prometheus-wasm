...
// ** wasm **
func CheckRulesWebWrapper() js.Func {
...
// ** wasm **
func CheckRulesWeb(data string, all bool, duplicateRules bool, fatal bool) (string, int) {
	failed := false
	hasErrors := false
	var returnedErrors []string
	rgs, errs := rulefmt.Parse([]byte(data))
	ls := lintConfig{
		all: all,
	...
	for _, e := range errs {
		fmt.Printf(e.Error())
		returnedErrors = append(returnedErrors, e.Error())
		return strings.Join(returnedErrors, ", "), failureExitCode
	}
	if n, errs := checkRuleGroups(rgs, ls); errs != nil {
		fmt.Printf("  FAILED:")
		returnedErrors = append(returnedErrors, "  FAILED:")
		for _, e := range errs {
			fmt.Printf(e.Error())
			returnedErrors = append(returnedErrors, e.Error())
		}
		...
		returnedErrors = append(returnedErrors, fmt.Sprintf("  SUCCESS: %d rules found\n", n))
	}
	if failed && hasErrors {
		return strings.Join(returnedErrors, ", "), failureExitCode
	}
